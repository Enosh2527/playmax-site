...
## Netlify preview says "Site not found"

If your preview URL shows Netlify's **Site not found** page (like
`deploy-preview-1--...netlify.app`), the problem is usually the Netlify project,
not the React app code.

Use this checklist:

1. Confirm the site still exists in the current Netlify account. If the site was
   deleted, old preview URLs stop working permanently.
2. Reconnect this repository in the active Netlify account (**Add new site → Import
   existing project**).
3. Keep build settings as:
   - Build command: `npm run build`
   - Publish directory: `dist`
4. Re-add all R2 environment variables in Netlify Site settings:
   `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_ACCOUNT_ID`, `R2_BUCKET`, and
   `R2_REGION`.
5. Redeploy and use the new Netlify domain generated by that account.

If the domain opens but page routes inside the app fail, add this SPA redirect rule
to `netlify.toml`:
